...
export const handleCourseFetch = async () => {
  try {
    ...
    if (!courseSchema.db.readyState) {
      throw new Error('Database connection is not ready');
    }

    // Return dummy data for testing if no database is available
    if (process.env.NODE_ENV === 'development' && !process.env.MONGO_URI) {
      return NextResponse.json({
        ...
        courses: [
          {
            COURSE_ID_PK: 1,
            COURSE_NAME: "Basic Quran Reading",
            STATUS: "ACTIVE",
            SORTING: 1,
          },
          {
            COURSE_ID_PK: 2,
            COURSE_NAME: "Tajweed Course",
            STATUS: "ACTIVE",
            SORTING: 2,
          },
          {
            COURSE_ID_PK: 3,
            COURSE_NAME: "Hifz Program",
            STATUS: "ACTIVE",
            SORTING: 3,
          }
        ],
    ...
    if (!courses) {
      console.error('No courses returned from database');
      return NextResponse.json({
    ...
    console.error("Error in handleCourseFetch:", error);
    
    return NextResponse.json({
      message: "Failed to fetch courses",
      error: error.message,
      courses: [] // Return empty array to trigger fallback courses
    }, { 
      status: 500 
    });
  }
